...
const uri = "mongodb+srv://adithivbsc22:UWUjQqigocvRLLc1@clusteradithi.6bto6mj.mongodb.net/Team&PlayersMongooseDB?retryWrites=true&w=majority";
...
async function connect_mongoose(player_num, players_array, team_num, teams_array) {
  try {
    ...
    const players_model = mongoose.model('Players',player_schema)
    const teams_model = mongoose.model('Teams',team_schema)

    //Inserting to the players collection
    let player
    for(let i=0;i<player_num;i++) {
      player = new players_model({
      PlayerId: players_array.players[i].id,
      Name: players_array.players[i].name,
    ...
    for(i=0;i<team_num;i++) {
      ...
      TeamId: teams_array.teams[i].id,
      Name: teams_array.teams[i].name,
      })
      await team.save();
    ...
    if(choice == 1) {
      ...
      await players_model.findOneAndUpdate({PlayerId: id}, {Name: name, Age: age, Team: team}, { upsert: true, useFindAndModify: false, validate: false });
      console.log("Player Updated \n");
    }
    else if(choice == 2) {
      let id = parseInt(prompt("Enter the id of the player to be deleted: "));
      await players_model.deleteOne({PlayerId: id});
      console.log("Player Deleted \n");
    }
    else if(choice == 3) {
        let id = parseInt(prompt("Enter the id of the team to be updated: "));
        let name = prompt("Enter the new team name: ");
        await teams_model.findOneAndUpdateOne({TeamId: id}, {Name: name}, { upsert: true, useFindAndModify: false, validate: false });
        console.log("Team Updated \n");  
    }
    else if(choice == 4) {
        let id = parseInt(prompt("Enter the id of the team to be deleted: "));
        await teams_model.deleteOne({TeamId: id});
...
function main() {
  ...
  connect_mongoose(player_num, players_array, team_num, teams_array);
}
